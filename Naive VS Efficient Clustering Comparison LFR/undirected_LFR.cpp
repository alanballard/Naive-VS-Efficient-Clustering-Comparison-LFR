...
//START MY EDIT: ADDED last 6 parameters for benchmark fct
//END MY EDIT
int benchmark(/*int **arr, int *comm,*/ bool excess, bool defect, int nodenum, double avgdeg, int maxdeg, double expdeg, double comsiz, double mixpar, int rep, int network_key) {
	/*START MY EDIT
...
//	cout << endl << endl << endl;
	...
	/*START MY EDIT
	//EDITED TO MAKE THE OUTPUT RUN A LITTLE FASTER
	cout<<"\n**************************************************************"<<endl;
...
*/
	...
	*/
	...
	//START MY EDIT
	//NOTE!!: Output files now save to their own folders, depending on type. These folders must be created in advance, 
	//or the files will not be saved. Folders should be labeled "network", "community" and "statistics".
	std::ostringstream fname1;
	ofstream network;
	//fname1 << "network\\network_" << nodenum << "_" << avgdeg << "_" << maxdeg << "_" << expdeg << "_" << comsiz << "_" << mixpar << "_" << rep << ".txt";
	fname1 << "undirected_LFR_network.dat";
	network.open(fname1.str().c_str());
	for (int u = 0; u<en.size(); u++) {
		set<int>::iterator it = en[u].begin();
		int uu = 0;
		while (it != en[u].end())
			//network << u + 1 << "\t" << *(it++) + 1 << endl;
			network << u << "\t" << *(it++) << endl;
		/*arr[u][*(it++)]=1;*/
	}
	network.close();
	...
	/*START MY EDIT
	// This is is how to format the suffix for the fast community alogrithm
	//Basically, it just needs to end in ".pairs" but since it (currently) has
	//to be called from the command-line, adding all of the parameters to the
	//file name is tedious
	std::ostringstream fname99;
	ofstream network1;
	fname99 << "network" << ".pairs";
	network1.open(fname99.str().c_str());
	for (int u=0; u<en.size(); u++) {
	set<int>::iterator it=en[u].begin();
	int uu=0;
	while (it!=en[u].end())
	network1<<u+1<<"\t"<<*(it++)+1<<endl; //54\t91\n
	}
	network1.close();
	END MY EDIT*/
	...
	ofstream community;
	int numberofgroups = 1; /*will return the largest group number=the number of groups in the LFR network*/
	//fname2 << "community\\community_" << nodenum << "_" << avgdeg << "_" << maxdeg << "_" << expdeg << "_" << comsiz << "_" << mixpar << "_" << rep << ".txt";
	fname2 << "undirected_LFR_community.dat";
	community.open(fname2.str().c_str(), ofstream::app);
	community << network_key << ",";
	for (int i = 0; i<member_list.size(); i++){
		//community << i + 1 << "\t" << member_list[i] + 1 << endl;
//		community << i << "\t" << member_list[i] + 1 << endl;
		community << member_list[i] + 1;
		if (i < member_list.size() - 1) { community << ","; }
		if (member_list[i]>numberofgroups){ numberofgroups = member_list[i]; }
		//comm[i] = member_list[i] + 1;
	}
	community <<endl;
//Writing generated network stats to screen
//	cout << "-----------------------------------------------------------" << endl;
...
//	cout << "p_in: " << density << "\tp_out: " << sparsity << endl;
	...
	ofstream statistics;
	//fname3 << "statistics\\statistics_" << nodenum << "_" << avgdeg << "_" << maxdeg << "_" << expdeg << "_" << comsiz << "_" << mixpar << "_" << rep << ".txt";
	fname3 << "undirected_LFR_statistics.dat";
	statistics.open(fname3.str().c_str());
	statistics << "degree distribution (degree-occurrences)" << endl;
	log_histogram(degree_seq, statistics, 10);
	statistics << endl << "--------------------------------------" << endl;
	statistics << "community distribution (size-occurrences)" << endl;
	histogram(num_seq, statistics);
	statistics << endl << "--------------------------------------" << endl;
	statistics << "mixing parameter" << endl;
	not_norm_histogram(double_mixing, statistics, 20, 0, 1.1);
/*
...
int main2() {
	...
	int nodenum_s = 50;                          //Number of nodes
	int nodenum_l = 50;
	int nodenum_r = 1;

	double avgdeg_s = 10;                          //Avg. Degree <k> in Fig. 5,
	double avgdeg_l = 10;                          //Benchmark graphs for testing community detection algorithms
	double avgdeg_r = 1;                           //Lancichinetti, Fortunato, Radicchi

	int maxdeg_s = 20;                             //Not really discussed in the paper. 
	int maxdeg_l = 20;                             //I've kept it constant here.
	int maxdeg_r = 1;

	double expdeg_s = 3;							// exponent of the degree sequence of the nodes (Gamma)
	double expdeg_l = 3;
	double expdeg_r = 1;

	double comsiz_s = 2;							// exponent for the pdf of the community sizes (Beta)
	double comsiz_l = 2;
	double comsiz_r = 1;

	double mixpar_s = 0.3;                          //Mixing parameter myu in Fig. 5, 
	double mixpar_l = 0.3;                          //Benchmark graphs for testing community detection algorithms
	double mixpar_r = 0.1;                            //Lancichinetti, Fortunato, Radicchi

	int rep = 1;                                      //Number of replicates at each combination of parameters

	//Create file containing parameter/rep information
	/*
	ofstream parameter("parameter.dat");
	parameter << nodenum_s << "\t" << nodenum_l << "\t" << nodenum_r
	<< "\t" << avgdeg_s << "\t" << avgdeg_l << "\t"
	<< avgdeg_r << "\t" << maxdeg_s << "\t" << maxdeg_l
	<< "\t" << maxdeg_r << "\t" << expdeg_s << "\t"
	<< expdeg_l << "\t" << expdeg_r << "\t" << comsiz_s
	<< "\t" << comsiz_l << "\t" << comsiz_r << "\t" << mixpar_s << "\t" << mixpar_l << "\t" << mixpar_r << "\t" << rep;
	parameter.close();
	//
	int iteration=0;
	for (int h=1; h<=rep; h++){
	for (int i=nodenum_s; i<=nodenum_l; i=i+nodenum_r){
	for (double j=avgdeg_s; j<=avgdeg_l; j=j+avgdeg_r){
	for (int k=maxdeg_s; k<=maxdeg_l; k=k+maxdeg_r){
	for (double l=expdeg_s; l<=expdeg_l; l=l+expdeg_r){
	for (double m=comsiz_s; m<=comsiz_l; m=m+comsiz_r){
	for (double n=mixpar_s; n<=mixpar_l; n=n+mixpar_r){
	iteration++;
	cout << "Iteration: "<<iteration<<endl;
	cout << "REP: " << h << endl;
	cout << "Creating network at levels: " << endl;
	cout << "[nodenum, avgdegree, maxdeg, expdeg, comsiz, mixpar]=[ "<<i<<", "<<j<<", "<<k<<", "<<l<<", "<<m<<", "<<n<<" ]"<<endl;
	benchmark(excess, defect, i, j, k, l, m, n, h);
	}
	}
	}
	}
	...
	//START MY EDIT: I added the last 7 parameters to the benchmark fct call
	//benchmark(excess, defect, nodenum, avgdeg, maxdeg, expdeg, comsiz, mixpar, rep);
	//END MY EDIT
	*/
	return 0;
}
