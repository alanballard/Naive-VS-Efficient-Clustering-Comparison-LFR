...
#define _CRT_SECURE_NO_WARNINGS 1; //ALAN to diable ctime warning
# define _CRT_NONSTDC_NO_DEPRECATE 1; 

#include "graph_binary.h"
#include "louvain.h"
//#include <unistd.h> ALAN
#include <io.h> //ALAN
...
vector<int> louvain_clustering(char *filename, int network_key, vector< vector<pair<int, int>> > links, int nb_links) {
 ...
 // time_t time_begin, time_end;
  ...
  delete q;
 time = (double)(end_new - start_new) / CLOCKS_PER_SEC;
  c.final_output(network_key, links, current_membership, quality, nb_links, time);
